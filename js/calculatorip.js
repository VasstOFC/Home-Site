...
function convertnum_hex(cform){
    ...
    cform.num.value=tmpvar; // show user
    if (! cform.num.value.match(/^[x0-9a-fA-F]+$/)){
        cform.errortxt.value = 'ERROR: invalid hex number';
        return;
    }
...
function convertnum_bin(cform){
    ...
    if (! cform.num.value.match(/^[01]+$/)){
        cform.errortxt.value = 'ERROR: invalid binary number';
        return;
    }
    //convert bin to dec:
...
function convertnum_dec(cform){
    ...
    if (! cform.num.value.match(/^\d+$/)){
        cform.errortxt.value = 'ERROR: invalid decimal number';
        return;
    }
